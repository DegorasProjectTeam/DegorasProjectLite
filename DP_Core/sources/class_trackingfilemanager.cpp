#include "includes/class_trackingfilemanager.h"
...
#include "includes/class_calibrationfilemanager.h"
...
SalaraInformation TrackingFileManager::readTracking(const QString &track_name, const QString &track_dirpath,
...
SalaraInformation TrackingFileManager::readTracking(const QString &track_name, Tracking &track)
{
...
SalaraInformation TrackingFileManager::writeTracking(const Tracking& track,
                                                     ...
                                                     const QString& filename )
{
    ...
    if (dest_dir.isEmpty())
    {
        QString current_path = SalaraSettings::instance().getGlobalConfigString(
                    "SalaraProjectDataPaths/SP_CurrentObservations");
        QString hist_path = SalaraSettings::instance().getGlobalConfigString(
...
SalaraInformation TrackingFileManager::removeTracking(const QString &track_name)
{
    SalaraInformation errors;
    QDate date_start(TrackingFileManager::startDate(track_name));
    QString current_filepath = SalaraSettings::instance().getGlobalConfigString(
                "SalaraProjectDataPaths/SP_CurrentObservations") + '/' + track_name;
    QString hist_filepath = SalaraSettings::instance().getGlobalConfigString(
...
SalaraInformation TrackingFileManager::removeCurrentTracking(const QString &track_name)
{
    SalaraInformation errors;
    QString current_filepath = SalaraSettings::instance().getGlobalConfigString(
                "SalaraProjectDataPaths/SP_CurrentObservations") + '/' + track_name;
...
SalaraInformation TrackingFileManager::readTrackingDir(const QString &dir, const QString& calib_path,
...
SalaraInformation TrackingFileManager::readTrackingDir(const QString &dir, std::vector<Tracking> &tracks)
{
...
QString TrackingFileManager::findTracking(const QString &track_name)
{
    ...
    if (date_start.isValid())
    {
        QString hist_trackpath =
                SalaraSettings::instance().getGlobalConfigString("SalaraProjectDataPaths/SP_HistoricalObservations");
                                               ...
                                               const QString &object_norad, const QString &cfg_id, const QString &dir)
{
    ...
    QString hist_trackpath = dir.isEmpty() ?
            SalaraSettings::instance().getGlobalConfigString("SalaraProjectDataPaths/SP_HistoricalObservations") : dir;
...
QDate TrackingFileManager::startDate(const QString &track_name)
{
    ...
    if (splitted.size() > 2)
    {
        ...
        if (12 == splitted[2].size())
            ...
            date_string = "20" + splitted[2].leftRef(6);
        date = QDate::fromString(date_string, "yyyyMMdd");
    }
    return date;
}

SalaraInformation TrackingFileManager::readTrackingFromFile(const QString &file_path, const QString &calib_path, Tracking &track)
{
    QFile track_file(file_path);
    // Check if file could be opened.
    if(!track_file.open(QIODevice::ReadOnly | QIODevice::Text))
        return SalaraInformation({TrackingFileManager::ErrorEnum::TRACKFILE_NOT_OPEN,
    ...
    else
    {
        ...
        // Stats
        track.stats_rfrms = StatsFileManager::fromJson(track_jsondocument[kStatsRFRMSKey].toObject());
        track.stats_1rms = StatsFileManager::fromJson(track_jsondocument[kStats1RMSKey].toObject());
        ...
        for (const auto& elem : std::as_const(array))
        {
            ...
            Calibration calib;
            SalaraInformation e = calib_path.isEmpty() ?
...
SalaraInformation TrackingFileManager::writeTrackingPrivate(const Tracking &track, const QString &filepath)
{
    ...
    // Stats
    track_object.insert(kStatsRFRMSKey, StatsFileManager::toJson(track.stats_rfrms));
    track_object.insert(kStats1RMSKey, StatsFileManager::toJson(track.stats_1rms));
    ...
    for (const auto& elem : track.ranges)
    {
        ...
        obj.insert(kStartKey, QString::fromStdString(dpslr::helpers::numberToStr(elem.start_time,17,12)));
    ...
    std::transform(track.tA.begin(), track.tA.end(), std::back_inserter(array),
                   [](const auto& a){return QString::fromStdString(dpslr::helpers::numberToStr(a, 18, 12));});
    if (!array.empty())
