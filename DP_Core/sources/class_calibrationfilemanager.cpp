...
SalaraInformation CalibrationFileManager::readCalibration(const QString &cal_name, const QString &dir_path, Calibration &calib)
{
...
SalaraInformation CalibrationFileManager::readCalibration(const QString &cal_name, Calibration &calib)
{
...
SalaraInformation CalibrationFileManager::writeCalibration(const Calibration& calib, const QString& dest_dir,
                                                           const QString& dest_file)
{
    ...
    if(!calib_file.open(QIODevice::WriteOnly | QIODevice::Text))
        return SalaraInformation({CalibrationFileManager::ErrorEnum::CALIBFILE_NOT_OPEN,
    ...
    calib_object.insert(kStatsRFRMSKey, StatsFileManager::toJson(calib.stats_rfrms));
    calib_object.insert(kStats1RMSKey, StatsFileManager::toJson(calib.stats_1rms));
...
SalaraInformation CalibrationFileManager::readCalibrationDir(const QString &dir, std::vector<Calibration> &calibs)
{
    Calibration c;
    SalaraInformation result;
...
SalaraInformation CalibrationFileManager::readLastCalib(Calibration &calib)
{
    QString hist_calpath =
            SalaraSettings::instance().getGlobalConfigString("SalaraProjectDataPaths/SP_HistoricalCalibrations");
...
QDateTime CalibrationFileManager::startDateTime(const QString &calib_name)
{
    ...
    if (splitted.size() > 2)
    {
        ...
        if (12 == splitted[2].size())
            ...
            datetime_string = "20" + splitted[2].leftRef(10);
        datetime = QDateTime::fromString(datetime_string, "yyyyMMddhhmm");
    }
    return datetime;
}

SalaraInformation CalibrationFileManager::readCalibrationFromFile(const QString &filepath, Calibration &calib)
{
    ...
    else
    {
        ...
        calib.stats_rfrms = StatsFileManager::fromJson(json[kStatsRFRMSKey].toObject());
        calib.stats_1rms = StatsFileManager::fromJson(json[kStats1RMSKey].toObject());
