...
SalaraInformation SalaraSettings::setGlobalConfig(const QString &path)
{
    // Clear the settings.
    delete this->settings_global;

    this->settings_global = new QSettings(path, QSettings::IniFormat);
    this->settings_global->setIniCodec("UTF-8");
    QFileInfo info(path);
...
SalaraInformation SalaraSettings::setApplicationConfig(const QString &path)
{
    // Clear the settings.
    delete this->settings_application;

    this->settings_application = new QSettings(path, QSettings::IniFormat);
    this->settings_application->setIniCodec("UTF-8");
    QFileInfo info(path);
...
SalaraInformation SalaraSettings::setStationData(const QString &path)
{
    // Register metatypes.
    ...
    if(!stationdatafile.open(QIODevice::ReadOnly | QIODevice::Text))
        return SalaraInformation({ErrorEnum::FILE_NOT_OPEN, "Station data json file not open."}, path);
    ...
    if (stationdatafile_jsondocument.isNull())
        return SalaraInformation({ErrorEnum::INVALID_JSON, "Station data json file invalid."}, path);
    ...
    if(array_coor.size()!=3)
        return SalaraInformation({ErrorEnum::INVALID_COORDINATES, "Station coordinates invalid."}, path);
    if(array_xyz.size()!=3)
        return SalaraInformation({ErrorEnum::INVALID_XYZ, "Station XYZ invalid."}, path);;
    x = array_xyz[0].toDouble();
    y = array_xyz[1].toDouble();
    z = array_xyz[2].toDouble();
    latitude = array_coor[0].toDouble()*dpslr::math::pi/180.0;
    longitude = array_coor[1].toDouble()*dpslr::math::pi/180.0;
...
SalaraSettings &SalaraSettings::instance()
{
    static SalaraSettings salara_settings;
    return  salara_settings;
}

SalaraSettings::~SalaraSettings()
{
