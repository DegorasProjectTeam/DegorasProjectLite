...
void StaticResidualsPlot::fastPopulate(const std::vector<double>& xdata, const std::vector<double> &ydata,
                                       DataTypeEnum type, dpslr::math::PolyFitRobustMethod robust)
{   
    switch(type)
    {
        // All the points.
        ...
        case StaticResidualsPlot::DataTypeEnum::DATA:
            ...
            auto coefs = dpslr::math::polynomialFit(xdata, ydata, 9, {}, robust);
            for (std::size_t i = 0; i < xdata.size(); i++)
                adjust_points.push_back({xdata[i], dpslr::math::applyPolynomial(coefs, xdata[i])});
